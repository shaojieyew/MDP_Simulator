...
public class ExplorationWallerType7 extends Exploration {
	...
	public static boolean visitedEnd = false;
	//from rpi
	public  ExplorationWallerType7(boolean isTerminate){
		super(isTerminate);
	}
	
	//from simulator
	public ExplorationWallerType7(){
		super();
	}
	public ExplorationWallerType7(int startAtX, int startAtY){
	...
	public Message computeAction(){
		...
		if(waypointFound()&&endPointFound()&&(seconds>=getAutoTerminate_time()||isOkToTerminate()||mapDiscoveredRate>=getAutoTerminate_explore_rate())){
			if(endPointFound()&&(!finishHuggingWall)){
				visitedEnd =true;
			}
	...
	private int[] getNextWallHugCheckLocation( int x, int y,int direction,boolean turnedLeft, boolean turnedRight,int nextWallHugChecker){
		nextWallHugChecker++;
		...
		if(turnedLeft&&nMoveable!=0){
			...
			result=getNextWallHugCheckLocation(next_result[0],next_result[1],direction,false,turnedRight,nextWallHugChecker);
		}else{
			if(wMoveable!=0){
				result=getNextWallHugCheckLocation(result[0],result[1],robotsWest,true,turnedRight,nextWallHugChecker);
			}else{
				if(steps!=0){
					int next_result[]= computeForwardLocation(direction, x, y, steps);
					result=getNextWallHugCheckLocation(next_result[0],next_result[1],direction,turnedLeft,false,nextWallHugChecker);

				}else{
					result=getNextWallHugCheckLocation(result[0],result[1],robotsEast,turnedLeft,true,nextWallHugChecker);
				}
			}
		}
		return result;
	...
	private Message getNextWallHugLocation( int x, int y,int direction,ArrayList<String> instructions){
		...
		int nextWallHugChecker = 0;
		int []endResult = getNextWallHugCheckLocation(x,y,direction,testTurnedLeft,testTurnedRight,nextWallHugChecker);
