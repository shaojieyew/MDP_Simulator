...
public class ExplorationWallerType2R extends Exploration {
	...
	public Message computeAction(){
		...
		if(!finishHuggingWall){
			...
			if(testTurnedRight==true&&outofWallhugging(currentX,currentY,(int)tempDirection)){
				finishHuggingWall=true;
			}else{
	...
	private int[] calculateScoreOfVertexAndCompare(int[] place1, int[] place2){
		...
		long currentTimeStamp = System.currentTimeMillis();
    	long seconds = ((currentTimeStamp-Robot.getInstance().getExploringStartTime())/1000);
		if(m.getExploredTiles()[18][13]==1 && (seconds>=getAutoTerminate_time()||isOkToTerminate()||mapDiscoveredRate>=getAutoTerminate_explore_rate())){
			startWeightage = 1000000;
			exploreMoreWeightage=0;
			distanceWeightage=0;
			terminate();
