...
public class ExplorationWallerType1 extends Exploration {
	...
	public void cleanUpVar(){
		temporaryTurnRightOnWallhug=false;
	...
	private Message getNextWallHugLocation( int x, int y,int direction,ArrayList<String> instructions){
		...
		if(temporaryTurnRightOnWallhug){
			...
			if(intDegree<0){
				rrmovement= "L"+(intDegree*-1);
			}
			instructions.add(rrmovement);
			temporaryTurnRightOnWallhug=false;
			testTurnedLeft=true;
			direction = robotsSouth;
		...
		if(endResult[0]!=1||endResult[1]!=1||!endPointFound()){
			...
			if(testTurnedLeft&&nMoveable!=0){	
				...
				if(wMoveable!=0){
					...
					if(!isBackAllExplored(robotsWest,x,y)){
						temporaryTurnRightOnWallhug = true;
						degree = degreeToRotateToDirection(direction,  robotsEast);
