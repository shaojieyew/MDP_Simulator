...
public class ExplorationWallerType5 extends Exploration {
	...
	public static int maxStep = 1;
	int printCount=1;

	
	//from rpi
	public  ExplorationWallerType5(boolean isTerminate){
		super(isTerminate);
	}
	
	//from simulator
	public ExplorationWallerType5(){
		super();
	}
	public ExplorationWallerType5(int startAtX, int startAtY){
	...
	public Message computeAction(){
		...
		if(!finishHuggingWall){
			...
			if(testTurnedRight==true&&outofWallhugging(currentX,currentY,(int)direction)){
				...
				if(lastMovedBeforeCalibrate>=intervalForCalibrate){
					instructions = addCalibrationCommand(currentX,currentY,(int) direction,instructions, getNextWallHugInstruction(currentX,currentY,(int) direction));
		...
		if(finishHuggingWall){
			if(ExplorationWallerType1.autoTerminateAfterWallhug<=m.getExploredRate()){
				terminate();
			}
	...
	public void cleanUpVar(){
		...
		setAutoTerminate_explore_rate(Exploration.autoTerminate_explore_rate);
	}
	/*
	...
	private int[] getNextWallHugCheckLocation2( int x, int y,int direction,boolean turnedLeft, boolean turnedRight, int defaultX, int defaultY, int defaultDirection){
		int robotsNorth = (int) ((NORTH+direction)%360);
		int robotsEast = (int) ((EAST+direction)%360);
		int robotsWest = (int) ((WEST+direction)%360);
		int nMoveable = isDirectionMoveable(robotsNorth, x, y);
		...
		if(turnedLeft&&nMoveable!=0){
			...
			if(wMoveable!=0){
				result=getNextWallHugCheckLocation2(result[0],result[1],robotsWest,true,turnedRight, defaultX,  defaultY,defaultDirection);
			}else{
				if(steps!=0){
					int next_result[]= computeForwardLocation(direction, x, y, steps);
					result=getNextWallHugCheckLocation2(next_result[0],next_result[1],direction,turnedLeft,false, defaultX,  defaultY,defaultDirection);
	...
	private Message getNextWallHugLocation( int x, int y,int direction,ArrayList<String> instructions){
		...
		int []nextEndResult=null;
		if(nMoveable>0){
			nextEndResult = getNextWallHugCheckLocation2(x,y,direction,testTurnedLeft,testTurnedRight,x,y,direction);
		}
		if(nextEndResult!=null){
			float degree = degreeToRotateToDirection(direction,  nextEndResult[2]);
			direction =  nextEndResult[2];
			r.rotate(degree);
			int intDegree = Math.round(degree);
			String rmovement= "R"+intDegree;
			if(intDegree<0){
			...
			testTurnedLeft = nextEndResult[3]==1?true:false;
			testTurnedRight = nextEndResult[4]==1?true:false;
			
			
			 robotsNorth = (int) ((NORTH+direction)%360);
			 robotsEast = (int) ((EAST+direction)%360);
			 robotsWest = (int) ((WEST+direction)%360);
			 robotsSouth = (int) ((SOUTH+direction)%360);
			 nMoveable = isDirectionMoveable(robotsNorth, x, y);
			 wMoveable = isDirectionMoveable(robotsWest, x, y);
			 previousBlocked = getLeftBlocks(direction,x, y);
		}
		...
		if(endResult[0]!=1||endResult[1]!=1||!endPointFound()){
			...
			if(testTurnedLeft&&nMoveable!=0){	
				...
				if(wMoveable!=0){
					float degree = degreeToRotateToDirection(direction,  robotsWest);
						testTurnedLeft=true;
	...
	*/
	...
	private ArrayList<Position> whatTileCanBeDiscovered(int x, int y, boolean firstLayer){
		...
		for(float dir: allDirection){
			...
			RobotSensorSimulatorType1 r = new RobotSensorSimulatorType1();
	...
	private boolean isBackAllExplored(int direction, int x, int y){
		int[][]obstacles = m.getObstacles();
		int[][]explored = m.getExploredTiles();
		int checkX ;
		int checkY ;
	...
	public String geType() {
		return ExplorationFactory.EX_WALL5;
	}
}
